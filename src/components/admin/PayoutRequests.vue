            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table v-if="" class="table table-hover">
                        ...
                        <tbody>
                        <tr v-if="automaticPayoutRequests.length > 0" v-for="payout in automaticPayoutRequests" :key="payout.id">
...
<script>
    ...
    export default {
        ...
        methods:{
            getPayoutRequests(){
                let vm = this;
                axios.get(config.api_hostname + '/getPayoutRequests')
                    .then(response => {
            ...
            markAsDone(p){
                ...
                this.$swal({
                    ...
                    result => {
                        axios.delete(config.api_hostname + '/markPayoutDone',
                            {
                                data: payout
                            }).then(response => {
                                vm.payoutRequests = response.data.payouts;
