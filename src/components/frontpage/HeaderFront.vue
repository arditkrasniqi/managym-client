            ...
            <nav class="navbar navbar-expand-lg">
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    ...
                    <form class="form-inline my-2 my-lg-0 btn-left">
                        ...
                        <a href="javascript:void(0)" data-toggle="modal" data-target="#loginModal" id="loginModalBtn">{{ $t('Login') }}</a>
            ...
            <div class="search-form ml-auto mr-auto">
                <div class="row">
                    <div class="col-12 col-sm-8">
                        <input type="search" id="google-input" v-model="trainerSearch.city" class="form-control" :placeholder="$t('City to search in')">
                        <!--<select placeholder="Type of workout" class="form-control">-->
                    ...
                    <!--</div>-->
                    <div class="col-12 col-sm-4">
                        <button @click="searchTrainer" class="btn btn-block"><i
                                v-if="trainerSearch.buttonSpin && trainerList.length == 0"
                                class="fa fa-spinner fa-spin"></i><span
                                v-if="trainerList.length > 0 || !trainerSearch.buttonSpin">{{ $t('Search') }}</span></button>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <div class="row">
                                ...
                                <div class="login-form">
                                    <form @submit.prevent="login">
                                        <div class="form-group">
                                            <input data-vv-validate-on="blur" v-validate="'required|email'" v-model="auth.username"
                                                   name="loginUsername" data-vv-as="Email" type="text"
                                                   :placeholder="$t('Email')" class="form-control">
                                            <span v-show="errors.has('loginUsername')" class="text-danger">{{ errors.first('loginUsername') }}</span>
                                        </div>
                                        <div class="form-group">
                                            <input data-vv-validate-on="blur" v-validate="'required|min:6'" v-model="auth.password" name="loginPassword"
                                                   data-vv-as="Password" type="password" :placeholder="$t('Password')"
                                                   class="form-control">
                                            <span v-show="errors.has('loginPassword')" class="text-danger">{{ errors.first('loginPassword') }}</span>
                                        </div>
                                        <div class="form-group">
                                            <div class="row login-buttons">
                                                <div class="col-12 col-sm-6">
                                                    <p><a href="javascript:void(0)" @click="toggleLogin">{{ $t('Forgot password') }}?</a></p>
                            ...
                            <div v-else class="col-8 ml-auto mr-auto text-center">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" :placeholder="$t('Email')" v-model="recoveryEmail" class="form-control">
                                </div>
                                <div class="form-group">
    ...
    export default {
        ...
        mounted() {
            ...
            if (URL.get("affiliateLink")) {
                ...
                // axios.post(config.api_hostname + '/setAffiliateCookie',
                //     {
                //         affiliateLink:URL.get('affiliateLink')
                //     }).then(response => {
        ...
        methods: {
            ...
            searchTrainer: function () {
                if (this.trainerSearch.city !== "") {
                    ...
                    this.trainerSearch.city = document.getElementById("google-input").value;
                    this.$store.dispatch("setTrainers", this.trainerSearch.city);
                }
            },
            login() {
                // this.auth.firstname = "_";
                // this.auth.lastname = "_";
                // this.auth.data = "_";
                let vm = this;
                this.$validator.validateAll().then(result => {
                    // if(vm.loginObj.username !== null && vm.loginObj.password){
                    //     result = true;
                    // }
                    if (result) {
