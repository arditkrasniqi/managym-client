                ...
                <div v-if="showUploadForm" class="col-12">
                    <form @submit.prevent="storeCertificates">
                        <div class="form-group">
                            <label>{{$t('Title')}}</label>
                            <input v-validate="'required'" v-model="certificateTitle" type="text" name="title" :data-vv-as="$t('Title')" class="form-control">
                            <span v-show="errors.has('title')" class="text-danger">{{ errors.first('title') }}</span></div>
                        <div class="form-group">
                            <label>{{$t('Description')}}</label>
                            <textarea v-validate="'required'" v-model="certificateDesc" name="description" :data-vv-as="$t('Description')" class="form-control certificate-description"></textarea>
                            <span v-show="errors.has('description')"
                                  class="text-danger">{{ errors.first('description') }}</span></div>
                        <div class="form-group">
                            <label>{{$t('Choose certificate')}}</label>
                            <input v-validate="'required'" accept=".pdf,.doc,.docx,.xls,.ppt" id="fileUploads" name="certificate" :data-vv-as="$t('Certificate')" type="file" @change="fileChanged" class="form-control">
                            <span v-show="errors.has('certificate')" class="text-danger">{{ errors.first('certificate') }}</span></div>
                        <div class="form-group">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getCertificates() {
                let vm = this;
                axios.get(config.api_hostname + "/trainerCertificates").then(response => {
            ...
            storeCertificates() {
                this.$validator.validateAll().then(result => {
                    if(result){
                        ...
                        axios
                            .post(
                                config.api_hostname + "/storeCertificates",
                                this.form,
                                configuration
                            )
                            .then(response => {
                                axios
                                    .post(config.api_hostname + "/storeCertificateToDatabase", {
