            ...
            <div class="col-12 col-sm-9">
                <form @submit.prevent="becomeTrainer">
                    <div class="form-group">
                        ...
                        <input list="locationsList" v-validate="'required'" type="text" name="Location"
                            v-model="location" :placeholder="$t('Choose Location')" class="form-control"
                            @keypress="citySearchOnChange">
                    ...
                    <div class="form-group">
                        ...
                        <textarea v-validate="'required'" name="Description" class="form-control" v-model="description"
                            cols="30" rows="7" :placeholder="$t('Describe yourself as a trainer')"></textarea>
                        <span v-show="errors.has('Description')"
                            class="text-danger">{{ errors.first('Description') }}</span>
                    </div>
                    <div class="form-group">
                        <label>{{ $t('Legal Terms') }}</label>
                        <textarea v-validate="'required'" class="form-control" name="LegalTerms"
                            :data-vv-as="$t('Legal Terms')" cols="30" rows="7" v-model="legalTerms"
                    ...
                    <!--<div class="form-group">-->
                    <!--<label>Price</label>-->
                    <!--<input v-validate="'required|numeric'" type="number" name="PerHour"-->
                    ...
                    <div class="form-group">
                        ...
                        <div class="row">
                            <div class="col-6">
                                <input v-validate="'required|numeric'" name="From" type="number" class="form-control"
                                    v-model="from" :placeholder="$t('From')">
                                <span v-show="errors.has('From')" class="text-danger">{{ errors.first('From') }}</span>
                            </div>
                            <div class="col-6">
                                <input v-validate="'required|numeric'" type="number" name="To" class="form-control"
                                    v-model="to" :placeholder="$t('To')">
                        ...
                        <div class="row">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center" v-for="plan in trainerPlans">
                                                <span v-if="plan.monthlyPay == null">{{ currency }} 0</span>
                                                <span v-else>{{ currency }} {{ plan.monthlyPay }}</span>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>{{ $t('Yearly Cost') }}</td>
                                            <td class="text-center" v-for="plan in trainerPlans">
                                                <span v-if="plan.yearlyPay == null">{{ currency }} 0</span>
                                                <span v-else>{{ currency }} {{ plan.yearlyPay }}</span>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td></td>
                                            <td class="text-center">
                                                <button class="btn btn-sm primary-bg"
                                                    @click="goToCard(1)">{{ $t('Choose Plan') }}
                                                </button>
                                            </td>
                                            <td class="text-center">
                                                <button class="btn btn-sm primary-bg"
                                                    @click="goToCard(2)">{{ $t('Choose Plan') }}
                                                </button>
                                            </td>
                                            <td class="text-center">
                                                <button class="btn btn-sm primary-bg"
                                                    @click="goToCard(3)">{{ $t('Choose Plan') }}
                                                </button>
                                            </td>
                                            <td class="text-center">
                                                <button class="btn btn-sm primary-bg"
                                                    @click="goToCard(4)">{{ $t('Choose Plan') }}
                                                </button>
                                            </td>
                                        </tr>
                                    </tbody>
                            ...
                            <div v-else class="col-12">
                                <div class="row">
                                    <div class="col-7">
                                        <div class="form-group">
                                            <input v-validate="'required|credit_card'" type="text" name="credit_card"
                                                :data-vv-as="$t('Credit Card Number')" v-model="plan.number"
                                                class="form-control" :placeholder="$t('Credit Card Number')">
                                            <span v-show="errors.has('credit_card')"
                                                class="text-danger">{{ errors.first('credit_card') }}</span>
                                        </div>
                                        <div class="form-group">
                                            <input name="cvc" data-vv-as="CVC" v-validate="'required'" type="text"
                                                v-model="plan.cvc" class="form-control"
                                                :placeholder="$t('Security Code')">
                                            <span v-show="errors.has('cvc')"
                                                class="text-danger">{{ errors.first('cvc') }}</span>
                                        </div>
                                        <div class="form-group">
                                            <input v-validate="'required|min_value:2018'" name="exp_year"
                                                :data-vv-as="$t('Year')" type="number" v-model="plan.exp_year"
                                                class="form-control" :placeholder="$t('Year (YYYY)')" min="1" max="12">
                                            <span v-show="errors.has('exp_year')"
                                                class="text-danger">{{ errors.first('exp_year') }}</span>
                                        </div>
                                        <div class="form-group">
                                            <input v-validate="'required|min_value:1|max_value:12'" type="number"
                                                name="exp_month" :data-vv-as="$t('Month')" v-model="plan.exp_month"
                                                class="form-control" :placeholder="$t('Month (MM)')" min="2018">
                                    ...
                                    <div class="col-5">
                                        <div class="row">
                                            <div class="col-6">
                                                <h5>{{ $t('Pay monthly') }}</h5>
                                                <div class="row">
                                                    <h1 class="pay-price col-12">{{ currency }} {{ plan.monthCost }}
                                                    </h1>
                                                </div>
                                            ...
                                            <div class="col-6">
                                                <h5>{{ $t('Pay yearly') }}</h5>
                                                <div class="row">
                                                    <h1 class="pay-price col-12">{{ currency }} {{ plan.yearCost }}</h1>
                                                </div>
...
export default {
    ...
    methods: {
        citySearchOnChange: debounce(async function (e) {
            ...
            const client = new AutoComplete(this.location)
            await client.init();
        ...
        becomeTrainer() {
            this.$validator.validateAll().then(result => {
                if (result) {
                    ...
                    if (!this.locationsList.find(item => item.name == this.location)) {
        ...
        async trainerRequest() {
            const client = new Geocode(this.location)
            await client.init();
            const coords = client.getCoords();

            this.latitude = coords.lat;
            this.longitude = coords.lon;
            this.req(coords.lat, coords.lon);
        },
        req(lat, lng) {
            let vm = this;
            vm.buttonSpin = true;
