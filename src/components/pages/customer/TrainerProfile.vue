    ...
    <div v-if="trainer.details !== ''" class="row">
        ...
        <div class="col-12 col-lg-4 col-xl-3">
            <div class="card trainer-info">
                <img class="mob-profile-picture card-img-top" :src="storage + trainer.details.avatar"
                    alt="Profile Picture">
                <div class="card-header desk">
                    <!--<center>-->
                    <img class="card-header-profile-picture" :src="storage + trainer.details.avatar"
                        alt="Profile Picture">
                    <div>
                        ...
                        <br />
                        <p>{{ $t('Available') }}: {{ trainer.details.availableFrom }} -
                ...
                <div class="card-block">
                    <div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                        <a @click="setActiveModule('about')" class="nav-link active" id="about-tab" data-toggle="pill"
        ...
        <div class="col-12 col-lg-8 col-xl-9">
            ...
            <div class="card profile-left" id="appointment" v-if="activeModule === 'makeAppointment'">
                <!--<div class="card profile-left" id="appointment"
                v-if="trainer.details.calendarAccess && activeModule === 'makeAppointment'">-->
                ...
                <div class="card-block">
                    <form v-if="trainer.details.calendarAccess" @submit.prevent="sendAppointmentRequest">
                        <div class="form-group">
                            ...
                            <div class="row">
                                <div class="col-6">
                                    <datepicker :disabled="disabled" v-validate="'required|date_format:YYYY-MM-DD'"
                                        name="startDate" :data-vv-as="$t('Start Date')"
                                        placeholder="Appointment start Date" v-model="appointmentDate"></datepicker>
                                    <span v-show="errors.has('startDate')"
                                        class="text-danger">{{ errors.first('startDate') }}</span>
                                </div>
                                <div class="col-6">
                                    <input v-validate="'required'" type="text" :placeholder="$t('Start Time')"
                                        name="startTime" data-vv-as="Start Time" id="startTimepicker"
                                        class="form-control timepicker">
                                    <span v-show="errors.has('startTime')"
                                        class="text-danger">{{ errors.first('startTime') }}</span>
                                </div>
                            </div>
                        ...
                        <div v-if="1 > 2" class="form-group">
                            ...
                            <div class="row">
                                <div class="col-6">
                                    <datepicker :disabled="disabled" v-validate="'required|date_format:YYYY-MM-DD'"
                                        :placeholder="$t('Appointment end Date')" name="endDate" data-vv-as="End Date"
                                        v-model="appointmentEndDate"></datepicker>
                                    <span v-show="errors.has('endDate')"
                                        class="text-danger">{{ errors.first('endDate') }}</span>
                                </div>
                                <div class="col-6">
                                    <input v-validate="'required'" type="text" :placeholder="$t('End Time')"
                                        name="endTime" data-vv-as="End Time" id="endTimepicker"
                                        class="form-control timepicker">
                                    <span v-show="errors.has('endTime')"
                                        class="text-danger">{{ errors.first('endTime') }}</span>
                                </div>
                            </div>
                            <span v-show="errors.has('AppointmentEndDate')"
                                class="text-danger">{{ errors.first('AppointmentEndDate') }}</span>
                        ...
                        <div class="form-group">
                            ...
                            <select id="trainerProgramsList" v-validate="'required|numeric'" :data-vv-as="$t('Program')"
                                name="program" v-model="trainerProgram" class="form-control">
                        ...
                        <div class="form-group">
                            <div class="row">
                                <div class="col-sm-4 col-lg-3 text-right">
                                    <a v-if="trainer.details.userId === userStore.info.id" disabled
                                        class="disabled btn btn-block primary-bg">
                                        <i class="fa"
                    ...
                    <p v-else>
                        {{ trainer.details.firstname }} {{ trainer.details.lastname }}
                        {{ $t('is not available for appointment') }}
                    </p>
                </div>
            </div>
            ...
            <trainer-profile-calendar :trainerId="trainer.details.userId" v-if="activeModule === 'todayAppointments'">
            </trainer-profile-calendar>
        </div>
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        loadPrograms() {
            ...
            let vm = this;
            axios.get(process.env.api_hostname + '/getProgramsByTrainerId/' + vm.trainer.details.userId).then(response => {
        ...
        getTrainer() {
            let vm = this;
            axios
                .post(process.env.api_hostname + "/getTrainerById", { id: this.trainerId })
                .then(response => {
