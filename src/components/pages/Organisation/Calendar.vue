        ...
        <div class="row" v-if="orgRole === 'client'">
            <div class="col-10 col-sm-6 col-md-4">
                <select id="calendarFilter" class="form-control" v-model="filterCalendarByProgram.current">
                    ...
                    <option v-for="program in filterCalendarByProgram.list" :value="program.id">{{ program.title }}
                    </option>
                </select>
            </div>
            <div class="col-2"><i v-if="filterCalendarByProgram.spinner"
                    class="filter-spinner fa fa-spinner fa-spin"></i></div>
        </div>
        <div class="row" v-else>
            <div class="col-10 col-sm-6 col-md-4">
                <select id="calendarFilterByTrainer" class="form-control" v-model="filterCalendarByTrainer.current">
                    ...
                    <option v-for="trainer in filterCalendarByTrainer.list" :value="trainer.id">{{ trainer.firstname }}
                        {{ trainer.lastname }}</option>
                </select>
            </div>
            <div class="col-2"><i v-if="filterCalendarByTrainer.spinner"
                    class="filter-spinner fa fa-spinner fa-spin"></i></div>
        </div>
            ...
            <div class="card">
                <div class="calendar-item card-block">
                    <full-calendar @dayClick="dayClick" @eventClick="eventClick" v-if="fcEvents != null"
                        :events="fcEvents" locale="en"></full-calendar>
                </div>
            </div>
        </div>
                                        ...
                                        <div class="row">
                                            <div class="col-6">
                                                <input name="exp_month" v-validate="'required|min_value:1|max_value:12'"
                                                    ...
                                                    :placeholder="$t('Month (MM)')" min="1" max="12"
                                            ...
                                            <div class="col-6">
                                                <input name="exp_year" v-validate="'required|min_value:2018'"
                                                    ...
                                                    :placeholder="$t('Year (YYYY)')" min="2018"
                                ...
                                <div class="col-12">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select id="trainerProgramsList" v-validate="'required|numeric'"
                                            ...
                                            <option
                                                v-for="trainerProgram in newAppointment.programs.trainerProgramsList"
...
export default {
    ...
    methods: {
        getOrganisationTrainers() {
            let vm = this;
            axios.get(`${process.env.api_hostname}/getOrganisationTrainers/${this.orgId}`)
                .then(response => {
        ...
        getOrganisationTrainersPrograms() {
            let vm = this;
            axios.get(`${process.env.api_hostname}/getOrganisationTrainersPrograms/${this.orgId}`)
                .then(response => {
        ...
        getTrainerPrograms(id) {
            let vm = this;
            axios.get(process.env.api_hostname + '/getProgramsByTrainerId/' + id).then(response => {
        ...
        getOrganisationTrainerAppointments(program = false, trainer = false) {
            let vm = this;
            axios.post(process.env.api_hostname + '/organisationTrainerAppointments', {
                orgId: this.orgId,
                programId: program,
                trainerId: trainer
            })
        ...
        getOrganisationAppointments() {
            ...
            axios.post(process.env.api_hostname + '/organisationAppointments', { orgId: this.orgId })
                .then(response => {
