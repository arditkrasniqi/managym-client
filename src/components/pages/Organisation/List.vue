        ...
        <div class="row">
                ...
                <div class="card">
                    ...
                    <div class="card-block">
                        <div class="form-group">
                            <a href="javascript:void(0)" @click="addMyOrg" class="btn primary-bg" v-if="!upgradeShow">{{$t('Add my Organisation')}}</a>
                            <a href="javascript:void(0)" @click="goToPlans" class="btn primary-bg" v-else>{{$t('View Trainer Plans and Upgrade')}}</a>
                            <!--<router-link :to="{name: 'organisations.create'}" class="btn primary-bg">Add my Organisation</router-link>-->
                        </div>
                        <div class="table-responsive">
                            <table v-if="organisations != null && organisations.length > 0" class="table table-hover table-sm">
                                <thead>
                                <tr>
                                    <th>{{$t('Name')}}</th>
                                    <th v-if="userStore.role != 'customer'" class="text-right">{{$t('Main Organisaion')}}</th>
                                ...
                                <tr v-for="organisation in organisations">
                                    ...
                                    <td v-if="userStore.role != 'customer'" class="icons">
                                        <a v-if="organisation.status == 1" href="javascript:void(0);" @click="makeMainOrganisation(organisation)"><i class="fa" :class="{'fa-toggle-off':mainOrg != organisation.id, 'fa-toggle-on':mainOrg == organisation.id}"></i></a>
                                    </td>
                                    <td class="icons">
            ...
            <div v-if="userStore.role == 'customer'" class="col-sm-12 col-md-6">
                <div class="card">
                    ...
                    <div class="card-block">
                        <div class="table-responsive">
                            <table v-if="otherOrganisations != null && otherOrganisations.length > 0" class="table table-hover table-sm">
                                <thead>
                                <tr>
                                    <th>{{$t('Name')}}</th>
                                    <th v-if="userStore.role != 'customer'" class="text-right">{{$t('Main Organisaion')}}</th>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            getOrganisations(){
                axios.get(config.api_hostname + '/userOrganisations').then(response => {
            ...
            getOtherOrganisations(){
                ...
                axios.get(config.api_hostname + `/customerOtherOrganisations/${userId}`)
                    .then(response => {
