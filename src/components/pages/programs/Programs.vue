<template>
    <div>
        ...
        <div>
                        ...
                        <table class="table-hover table">
                            ...
                            <tr v-for="program in trainerPrograms" :key="program.id">
                                ...
                                <td class="icons text-center">
                                    <a v-if="program.organisationProgram === 0" href="javascript:void(0)" @click="editProgramModal(program)">
                                        <i class="fa fa-edit"></i>
                                    </a>
                                </td>
                                <td class="icons text-center">
                                    <a v-if="program.organisationProgram === 0" href="javascript:void(0)" @click="removeProgram(program)">
            ...
            <p class="programs-note">{{$t('NOTE')}}:<br/>{{$t('Programs created by Organisation owners cant be edited nor removed')}}!</p>
        </div>
                    ...
                    <form @submit.prevent="editProgram">
                        <div class="modal-body">
                            <div class="form-group">
                                <label>{{$t('Title')}}</label>
                                <input v-validate="'required'" v-model="programToEdit.title" name="title" :data-vv-as="$t('Title')" type="text" class="form-control">
                                <span v-show="errors.has('title')" class="text-danger">{{ errors.first('title') }}</span>
                            </div>
                            <div class="form-group">
                                <label>{{$t('Description')}}</label>
                                <textarea v-validate="'required'" v-model="programToEdit.description" type="text" name="description" :data-vv-as="$t('Description')" class="form-control"></textarea>
                                <span v-show="errors.has('description')" class="text-danger">{{ errors.first('description') }}</span>
                            </div>
                            <div class="form-group">
                                <label>{{$t('Duration')}}</label>
                                <input readonly v-validate="'required'" v-model="programToEdit.duration" type="text" name="duration" :data-vv-as="$t('Duration')" placeholder="hh:mm" id="programDuration" class="timepicker form-control"></input>
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" v-model="programToEdit.price" v-validate="'required|decimal'" class="form-control" name="price" :data-vv-as="$t('Price')">
                                </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getTrainerPrograms() {
                let vm = this;
                axios.get(config.api_hostname + '/trainerPrograms')
                    .then(response => {
